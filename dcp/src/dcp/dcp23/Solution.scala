...
object Solution {
  ...
  @annotation.tailrec
  def subSolve(b: Board, end: Coordinates, currentSet: Set[Coordinates], visited: Set[Coordinates], curLen: Int): Option[Int] = {
    if (currentSet.isEmpty) None
    else {
      if (currentSet.contains(end)) Some(curLen)
      else {
        val nextSet = currentSet
          .flatMap(c => moveVectors.map(c + _))
          .filter(c => b.isAccessible(c) && !visited.contains(c))
          
        subSolve(b, end, nextSet, visited ++ currentSet, curLen + 1)
      }
    }
  }
}